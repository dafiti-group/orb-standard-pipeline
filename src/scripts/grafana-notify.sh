...
TIMESTAMP=$(date +%s)
TIMENOW=$(date -R)
MESSAGE="{ \"what\": \"App:${CIRCLE_PROJECT_REPONAME}-${COUNTRY}\", \"tags\": [ \"pipeline-notifications-${PARAMETER_ENV}\" ], \"when\": ${TIMESTAMP}, \"data\": \"Time: ${TIMENOW} Circleci link: ${CIRCLE_BUILD_URL} commit: ${LOCAL_GITHUB_URL}\" }"
