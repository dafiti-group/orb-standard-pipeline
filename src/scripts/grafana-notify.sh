...
TIMESTAMP=$(date +%s)
TIMENOW=$(date -R)
MESSAGE=$(echo "{ \"what\": \"App:${CIRCLE_PROJECT_REPONAME}\", \"tags\": [ \"pipeline-notifications-${PARAMETER_ENV}\" ], \"when\": ${TIMESTAMP}, \"data\": \"Time: ${TIMENOW} Circleci link: ${CIRCLE_BUILD_URL} commit: ${LOCAL_GITHUB_URL}\" }")
